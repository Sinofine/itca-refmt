...
\begin{proof}
By (\ref{prop:3.1}) we have to show that $h: S^{-1} A \rightarrow B$, defined by
...
In particular, it follows from (\ref{prop:3.3}) that if $M^{\prime}$ is a submodule of $M$, the mapping $S^{-1} M^{\prime} \rightarrow S^{-1} M$ is injective and therefore $S^{-1} M^{\prime}$ can be regarded as a submodule of $S^{-1} M$. With this convention,
...
is $A$-bilinear, and therefore by the universal property \ref{prop:2.12} of the tensor product induces an $A$-homomorphism
...
\begin{proof}
Use (\ref{prop:3.5}) and the canonical isomorphisms of Chapter 2.
\end{proof}
...
\begin{proof}
Clearly i) $\Rightarrow$ ii) $\Rightarrow$ iii). Suppose iii) satisfied and $M \neq 0$. Let $x$ be a non-zero element of $M$, and let $\mathfrak{a}=\operatorname{Ann}(x) ; \mathfrak{a}$ is an ideal $\neq(1)$, hence is contained in a maximal ideal $\mathfrak{m}$ by \ref{cor:1.4}. Consider $x / l \in M_{\mathfrak{m}}$. Since $M_{\mathfrak{m}}=0$ we have $x / 1=0$, hence $x$ is killed by some element of $A-\mathfrak{m}$; but this is impossible since Ann $(x) \subseteq \mathfrak{m}$.
\end{proof}
...
\begin{proof}
\begin{enumerate}[i)]
  \item $\Rightarrow$ ii) by (\ref{prop:3.5}) and (2.20).
...
\[
\begin{aligned}
N \rightarrow P \text { injective } & \Rightarrow N_{\mathfrak{m}} \rightarrow P_{\mathfrak{m}} \text { injective, by (\ref{prop:3.9}) } \\
& \Rightarrow N_{\mathfrak{m}} \otimes_{A_{\mathfrak{m}}} M_{\mathfrak{m}} \rightarrow P_{\mathfrak{m}} \otimes_{A_{\mathfrak{m}}} M_{\mathfrak{m}} \text { injective, by \ref{prop:2.19} } \\
& \Rightarrow\left(N \otimes_{A} M\right)_{\mathfrak{m}} \rightarrow\left(P \otimes_{A} M\right)_{\mathfrak{m}} \text { injective, by (\ref{prop:3.7}) } \\
& \Rightarrow N \otimes_{A} M \rightarrow P \otimes_{A} M \text { injective, by (\ref{prop:3.9}). }
\end{aligned}
\]

\end{enumerate}
Hence $M$ is flat by \ref{prop:2.19}.
\end{proof}

\section{Extended and Contracted Ideals in Rings of Fractions}
Let $A$ be a ring, $S$ a multiplicatively closed subset of $A$ and $f: A \rightarrow S^{-1} A$ the natural homomorphism, defined by $f(a)=a / 1$. Let $C$ be the set of contracted ideals in $A$, and let $E$ be the set of extended ideals in $S^{-1} A$ (cf. \ref{prop:1.17}). If $\mathfrak{a}$ is an ideal in $A$, its extension $a^{e}$ in $S^{-1} A$ is $S^{-1} \mathfrak{a}$ (for any $y \in a^{e}$ is of the form $\sum a_{\mathfrak{l}} / s_{\text {, }}$, where $a_{\mathfrak{t}} \in \mathfrak{a}$ and $s_{\mathfrak{i}} \in S$; bring this fraction to a common denominator).
...
\begin{proof}
\begin{enumerate}[i)]
  \item Let $\mathfrak{b}$ be an ideal in $S^{-1} A$, and let $x / s \in \mathfrak{b}$. Then $x / 1 \in \mathfrak{b}$, hence $x \in \mathfrak{b}^{\mathfrak{c}}$ and therefore $x / s \in \mathfrak{b}^{c \varepsilon}$. Since $\mathfrak{b} \supseteq \mathfrak{b}^{c e}$ in any case \ref{prop:1.17}, it follows that $\mathfrak{b}=\mathfrak{b}^{c e}$.
  ...
  \item For sums and products, this follows from \ref{exc:1.18}; for intersections, from (\ref{cor:3.4}). As to radicals, we have $S^{-1} r(\mathfrak{a}) \subseteq r\left(S^{-1} \mathfrak{a}\right)$ from \ref{exc:1.18}, and the proof of the reverse inclusion is a routine verification which we leave to the reader.
\end{enumerate}
\end{proof}
...
is true provided the ideal $\mathfrak{b}$ is finitely generated: see (\ref{cor:3.15}).

\item The proof in \ref{prop:1.8} that if $f \in A$ is not nilpotent there is a prime ideal of $A$ which does not contain $f$ can be expressed more concisely in the language of rings of fractions. Since the set $S=\left(f^{n}\right)_{n>0}$ does not contain 0, the ring $S^{-1} A=A_{f}$ is not the zero ring and therefore by \ref{thm:1.3} has a maximal ideal, whose contraction in $A$ is a prime ideal $\mathfrak{p}$ which does not meet $S$ by (\ref{prop:3.11}); hence $f \notin \mathfrak{p}$.
\end{enumerate}
\end{remark}
...
\[
\begin{aligned}
S^{-1}(\operatorname{Ann}(M+N)) & =S^{-1}(\operatorname{Ann}(M) \cap \operatorname{Ann}(N)) \text { by }\ref{exc:2.2} \\
& =S^{-1}(\operatorname{Ann}(M)) \cap S^{-1}(\operatorname{Ann}(N)) \text { by (\ref{cor:3.4}) } \\
...
Hence it is enough to prove (\ref{prop:3.14}) for $M$ generated by a single element: then $M \cong A / \mathfrak{a}$ (as $A$-module), where $\mathfrak{a}=\operatorname{Ann}(M) ; S^{-1} M \cong\left(S^{-1} A\right) /\left(S^{-1} \mathfrak{a}\right)$ by (\ref{cor:3.4}), so that Ann $\left(S^{-1} M\right)=S^{-1} \mathfrak{a}=S^{-1}(\operatorname{Ann}(M))$.
\end{proof}
...
\begin{proof}
$(N: P)=$ Ann $((N+P) / N)$ by \ref{exc:2.2}; now apply (\ref{prop:3.14}).
\end{proof}
...
\begin{proof}
If $\mathfrak{p}=\mathfrak{q}^{c}$ then $\mathfrak{p}^{e c}=\mathfrak{p}$ by \ref{prop:1.17}. Conversely, if $\mathfrak{p}^{e c}=\mathfrak{p}$, let $S$ be the image of $A-\mathfrak{p}$ in $B$. Then $\mathfrak{p}^{e}$ does not meet $S$, therefore by (\ref{prop:3.11}) its extension in $S^{-1} B$ is a proper ideal and hence is contained in a maximal ideal $\mathfrak{m}$ of $S^{-1} B$. If $\mathfrak{q}$ is the contraction of $\mathfrak{m}$ in $B$, then $\mathfrak{q}$ is prime, $q \supseteq \mathfrak{p}^{*}$ and $q \cap S=\varnothing$. Hence $\mathfrak{q}^{c}=\mathfrak{p}$.
\end{proof}
...
Use this result and Nakayama's lemma to give a proof of \ref{cor:2.5} which does not depend on determinants. [If $M=a M$, then $S^{-1} M=\left(S^{-1} a\right)\left(S^{-1} M\right)$, hence by Nakayama we have $S^{-1} M=0$. Now use Exercise 1.]
...
\end{enumerate}
  ...
  \item Let $M$ be an $A$-module and $a$ an ideal of $A$. Suppose that $M_{\mathfrak{m}}=0$ for all maximal ideals $m \supseteq \mathfrak{a}$. Prove that $M=\mathfrak{a} M$. [Pass to the $A / \mathfrak{a}$-module $M / \mathfrak{a} M$ and use (\ref{prop:3.8}).]
...
\end{enumerate}
(For i) $\Rightarrow$ ii), use (\ref{prop:3.16}). ii) $\Rightarrow$ iii) is clear.
...
$B$ is said to be faithfully flat over $A$.
  ...
  \item Let $f: A \rightarrow B$ be a flat homomorphism of rings, let $q$ be a prime ideal of $B$ and let $\mathfrak{p}=q^{\mathfrak{c}}$. Then $f^{*}: \operatorname{Spec}\left(B_{q}\right) \rightarrow \operatorname{Spec}\left(A_{\mathfrak{p}}\right)$ is surjective. [For $B_{\mathfrak{p}}$ is fiat over $A_{\mathfrak{p}}$ by (\ref{prop:3.10}), and $B_{q}$ is a local ring of $B_{\mathfrak{p}}$, hence is flat over $B_{\mathfrak{p}}$. Hence $B_{q}$ is flat over $A_{\mathfrak{p}}$ and satisfies condition (3) of Exercise 16.]
