...
\section{Discrete Valuation Rings}\index{Discrete valuation ring}
...
An integral domain $A$ is a \emph{discrete valuation ring}\index{Discrete valuation ring} if there is a discrete valuation $v$ of its field of fractions $K$ such that $A$ is the valuation ring of $v$. By \eqref{prop:5.18}, $A$ is a local ring, and its maximal ideal $m$ is the set of all $x \in K$ such that $v(x)>0$.
...
A ring satisfying the conditions of \eqref{thm:9.3} is called a \emph{Dedekind domain}\index{Dedekind domain}.
