...
A prime ideal\index{Ideal!prime}\index{Prime ideal(s)} in a ring $A$ is in some sense a generalization of a prime number.
...
A primary decomposition\index{Primary!decomposition} of an ideal $a$ in $A$ is an expression of $a$ as a
...
$\mathfrak{a}$, or to be associated with $a$.\index{Prime ideal(s)!associated} The ideal $a$ is primary if and
...
minimal or isolated prime ideals\index{Prime ideal(s)!isolated} belonging to $a$. The others are called
embedded prime ideals.\index{Prime ideal(s)!embedded} In the example above, $\mathfrak{p}_{2}=(x, y)$ is
embedded.
...
$A$ ) is finite.
  ...
  symbolic power\index{Symbolic power} of $\mathfrak{p}$ is defined to be the ideal (in the notation
  of Exercise 12)
\[
...
\begin{enumerate}[1.]
  \setcounter{enumi}{19}
  \item Let $M$ be a fixed $A$-module, $N$ a submodule of $M$. The radical\index{Radical!of a submodule} of
