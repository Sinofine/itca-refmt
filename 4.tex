...
ideal. An ideal $\mathfrak{q}$ in a ring $A$ is \textit{primary} if
$\mathfrak{q}\neq A$ and if
\[
...
\end{proposition}
\begin{proof}
  By \ref{prop:1.8} it is enough to show that $\mathfrak{p}=r(\mathfrak{q})$ is prime. Let
...
\begin{proof}
  ...
  has only one prime ideal, by \ref{prop:1.8}. Hence every element of $A/\mathfrak{a}$ is
...
\begin{proof}
  ...
  \mathfrak{p}_{j}$ by Lemma~\ref{lem:4.4}. Suppose $r(a: x)$ is prime; then by \eqref{prop:1.11} we have
...
$\mathfrak{p}_{2}^{2}$ is primary by ~\ref{prop:4.2}. So the prime ideals are
...
The prime ideals $\mathfrak{p}_{1}$ in ~\ref{thm:4.5} are said to belong to
...
\mathfrak{p}_{\mathfrak{k}}$. Hence by \ref{prop:1.11} we have $\mathfrak{p} \supseteq
...
embedded in the irreducible components. Thus in the example before ~\ref{prop:4.6} the
...
However, there are some uniqueness properties: see ~\ref{thm:4.10}.
\end{enumerate}
\end{remark}
...
~\ref{prop:4.7}. By \eqref{prop:1.15} we have $D=\bigcup_{x \neq 0} r(0: x)$; from the proof of
~\ref{thm:4.5}, we have $r(0: x)=\bigcap_{x \xi q_{1}} \mathfrak{p}_{j} \subseteq
\mathfrak{p}_{j}$ for some $j$, hence $D \subseteq \bigcup_{i-1}^{n}
\mathfrak{p}_{i}$. But also from ~\ref{thm:4.5} each $p_{i}$ is of the form $r(0: x)$ for
...
primary ideals correspond to primary ideals in the correspondence \ref{prop:3.11} between
...
\ref{prop:3.11}. Also from \ref{prop:3.11} we have $r\left(\mathfrak{q}^{e}\right)=r\left(S^{-1}
...
\end{proposition}
\begin{proof}
$S^{-1} \mathfrak{a}=\bigcap_{i=1}^{n} S^{-1} q_{i}$ by \ref{prop:3.11}
$=\bigcap_{i=1}^{m} S^{-1} q_{i}$ by ~\ref{prop:4.8}, and $S^{-1} q_{i}$ is $S^{-1}
...
\mathfrak{p}^{\prime} \notin \Sigma \Rightarrow \mathfrak{p}^{\prime} \neq \bigcup_{\mathfrak{p} \in \mathbb{\Sigma}} \mathfrak{p}(\text { by }\ref{prop:1.11}) \Rightarrow \mathfrak{p}^{\prime} \cap S \neq \varnothing .
\end{gathered}
\]

Hence, from ~\ref{prop:4.9}, we deduce
...
\end{corollary}
\begin{proof}[Proof of ~\ref{thm:4.10}]
...
State and prove the formulas for $r_{M}$ analogous to \ref{exc:1.13}.
...
Prove the analogues of ~\ref{lem:4.3} and ~\ref{lem:4.4}.
...
Prove the analogue of ~\ref{thm:4.5}, that the prime ideals $\mathfrak{p}_{1}$ depend
...
$M$. Show that they are also the prime ideals belonging to 0 in $M / N$.
  ...
  \item State and prove the analogues of ~\ref{prop:4.6}--~\ref{cor:4.11} inclusive. (There is no
