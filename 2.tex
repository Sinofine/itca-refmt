...
\index{Module}
Let $A$ be a ring (commutative, as always). An \textit{$A$-module} is an abelian group
...
$M / M'$ is the \textit{quotient}\index{Module!quotient} of $M$ by $M'$. The natural map of $M$ onto
...
If $f\colon M \to N$ is an $A$-module homomorphism, the \textit{kernel}\index{Kernel} of $f$ is the
set
\[
  \Ker(f)=\{x \in M: f(x)=0\}
\]
and is a submodule of $M$. The \textit{image}\index{Image} of $f$ is the set
\[
  \Img(f)=f(M)
\]
and is a submodule of $N$. The \textit{cokernel}\index{Cokernel} of $f$ is
\[
  \Coker(f)=N / \Img(f)
\]
...
We cannot in general define the \textit{product}\index{Module!product} of two submodules, but we can
...
An $A$-module is \textit{faithful}\index{Module!faithful} if $\Ann(M)=0$. If $\Ann(M)=\mathfrak{a}$,
...
$x_{i}$ are said to be a \textit{set of generators}\index{Generators} of $M$; this means that
...
said to be \textit{finitely generated}\index{Module!finitely-generated} if it has a finite set of generators.

\section{Direct sum and product}
\index{Module!direct sum}
\index{Module!direct product}
...
\end{corollary}
\begin{proof}
  Take $\phi=$ identity, $x=1+a_{1}+\cdots+a_{n}$ in \eqref{prop:2.4}.
\end{proof}
\begin{proposition}[Nakayama's Lemma]\label{prop:2.6}
  Let $M$ be a finitely generated $A$-module and $\mathfrak{a}$ an ideal of $A$
  contained in the Jacobson radical $\mathfrak{R}$ of $A$. Then $\mathfrak{a} M=M$ implies $M=0$.
\end{proposition}
\begin{proof}[First proof]
By \eqref{cor:2.5} we have $x M=0$ for some $x \equiv 1\pmod{\Re}$. By \eqref{prop:1.9}
...
\]
since $a_{n} \in \Re$, it follows from \eqref{prop:1.9} that $1-a_{n}$ is a unit in $A$.
...
\end{corollary}
\begin{proof}
  Apply \eqref{prop:2.6} to $M / N$, observing that
...
$M / \mathfrak{m} M$, hence $N+\mathfrak{m} M=M$, hence $N=M$ by \eqref{cor:2.7}.
\end{proof}
...
A sequence of type \eqref{es3} is called a {\itshape short exact
  sequence}. Any long exact sequence \eqref{es0} can be split up into
...
\begin{proposition}
  ...
  M' \stackrel{u}{\to} M \stackrel{v}{\to} M'' \to 0\tag{4}
    \end{equation}
be a sequence of A-modules and homomorphisms. Then the sequence \eqref{es4} is exact
...
\item Let
  \begin{equation}
    ...
    \tag{5}
  \end{equation}
be a sequence of A-modules and homomorphisms. Then the sequence \eqref{es5} is exact
...
suppose that \eqref{es4a} is exact for all $N$. First of all, since $\bar{v}$ is
...
sequence \eqref{es6} is exact, is a straightforward exercise in diagram-chasing which we
leave to the reader.\qedsymbol
\begin{remark}
\eqref{prop:2.10} is a special case of the exact homology sequence of homological
algebra.
\end{remark}
...
). The function $\lambda$ is \index{Additive function}\textit{additive} if, for each short exact sequence (3) in
...
We shall construct an $A$-module $T$, called the {\itshape tensor
  product}\index{Tensor product} of $M$ and $N$, with the property that the $A$-bilinear
...
it is non-zero. See the example after \eqref{prop:2.18}.
...
the proof of \eqref{prop:2.12} we have $\sum(x_{i}, y_{i}) \in D$, and therefore
...
given in \eqref{prop:2.12}, and the reader may safely forget it if he prefers. What is
...
mappings, and use the defining property \eqref{prop:2.12} or \eqref{prop:2.12a} to
...
{\itshape extension of scalars}\index{Scalars!extension of}.
\begin{proposition}
  \label{prop:2.17}
...
\end{proposition}
\begin{proof}
  Let $E$ denote the sequence \eqref{exp2}, and let $E \otimes N$ denote the
sequence \eqref{exp3}. Let $P$ be any $A$-module. Since \eqref{exp2} is exact, the sequence
$\Hom(E$, Hom $(N, P))$ is exact by \eqref{prop:2.9}; hence by \eqref{exp1} the sequence
$\Hom(E \otimes N, P)$ is exact. By \eqref{prop:2.9} again, it follows
that $E \otimes N$ is exact.
\end{proof}
\begin{remark}
\begin{enumerate}[i)]
\item Let $T(M)=M \otimes N$ and let $U(P)=\Hom(N, P)$. Then \eqref{exp1} takes the
...
$U$ is the right adjoint of $T$. The proof of \eqref{prop:2.18} shows that any functor
...
sequences, then $N$ is said to be a {\itshape flat}\index{Module!flat}\index{Flat} $A$-module.
...
exact sequences.
  \item[ii) $\Longleftrightarrow$ iii)]by \eqref{prop:2.18}.
...
$\sum x_{i}' \otimes y_{i}$ as an element of $M_{0}' \otimes N$. By \eqref{prop:2.14} there
...
canonical isomorphisms \eqref{prop:2.14}, \eqref{exc:2.15}.)
\end{exercise}
\section{Algebras}
...
injective by \eqref{prop:1.2} and therefore $K$ can be canonically identified with its
...
\]
This is $A$-linear in each factor and therefore, by \eqref{prop:2.12a},
induces an $A$-module homomorphism
\[
  B \otimes C \otimes B \otimes C \to D,
\]
hence by \eqref{prop:2.14} an $A$-module homomorphism
\[
  D \otimes D \to D
\]
and this in turn by \eqref{prop:2.11} corresponds to an $A$-bilinear mapping
\[
  \mu: D \times D \to D
\]
which is such that
...
\eqref{prop:2.19}. If $M$ is finitely 
...
\eqref{prop:2.19} again to reduce to the case where $\mathfrak{a}$ is a finitely
generated ideal.]
