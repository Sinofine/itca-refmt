...
\begin{proof}
$\Rightarrow$: By \ref{prop:8.1} we have $\operatorname{dim} A=0$. Let $\mathfrak{m}_{i}(1 \leqslant i \leqslant n)$ be the distinct maximal ideals of $A$ \ref{prop:8.3}. Then $\prod_{i=1}^{n} \mathfrak{m}_{i}^{k} \subseteq\left(\bigcap_{i=1}^{n} \mathfrak{m}_{i}\right)^{k}=\mathfrak{R}^{k}=0$. Hence by (6.11) $A$ is Noetherian.
...
\begin{enumerate}[i)]
  ...
  \item $\mathrm{m}^{\mathrm{n}}=0$ for some $n$, in which case $A$ is an Artin local ring.
\end{enumerate}
\end{proposition}
