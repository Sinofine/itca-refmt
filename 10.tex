...
\]
where $U$ runs through all neighborhoods of 0 in $G$, and so by \ref{lem:10.1} $\phi$ is injective if and only if $G$ is Hausdorff.
...
\end{aligned}
\]
and hence by $\ref{prop:2.10}$ an exact sequence
...
\end{corollary}
\begin{proof}
Apply \ref{prop:10.2} to the exact sequences
\[
0 \rightarrow \frac{G^{\prime}}{G^{\prime} \cap G_{n}} \rightarrow \frac{G}{G_{n}} \rightarrow \frac{G^{\prime \prime}}{p G_{n}} \rightarrow 0 .
\]
\end{proof}

In particular we can apply \ref{cor:10.3} with $G^{\prime}=G_{n}$, then $G^{\prime \prime}=G / G_{n}$ has the discrete topology so that $\hat{G}^{\prime \prime}=G^{\prime \prime}$. Hence we deduce
...
Taking inverse limits in \ref{cor:10.4} we deduce

\begin{proposition}\label{prop:10.5}
$G \cong \hat{G}$.
\end{proposition}

If $\phi: G \rightarrow \hat{G}$ is an isomorphism we shall say that $G$ is complete. Thus \ref{prop:10.5} asserts that the completion of $G$ is complete. Note that our definition of complete includes Hausdorff (by \ref{lem:10.1}).

The most important class of examples of topological groups of the kind we are considering are given by taking $G=A, G_{n}=a^{n}$, where $\mathfrak{a}$ is an ideal in a ring $A$. The topology so defined on $A$ is called the a-adic topology, or just the $\mathfrak{a}$-topology. Since the $\mathfrak{a}^{n}$ are ideals, it is not hard to check that with this topology $A$ is a topological ring, i.e. that the ring operations are continuous. By \ref{lem:10.1} the topology is Hausdorff $\Leftrightarrow \cap \mathfrak{a}^{n}=(0)$. The completion $\hat{A}$ of $A$ is again a topological ring; $\phi: A \rightarrow \hat{A}$ is a continuous ring homomorphism, whose kernel is $\cap \mathfrak{a}^{n}$.
...
\item $\Rightarrow$ i): by Hilbert's basis theorem \ref{cor:7.6}. Let $A$ be a ring (not graded), $\mathfrak{a}$ an ideal of $A$. Then we can form a graded ring $A^{*}=\bigoplus_{n=0}^{\infty} a^{n}$. Similarly, if $M$ is an $A$-module and $M_{n}$ is an a-filtration of $M$, then $M^{*}=\bigoplus_{n} M_{n}$ is a graded $A^{*}$-module, since $a^{m} M_{n} \subseteq M_{m+n}$.

If $A$ is Noetherian, $a$ is finitely generated, say by $x_{1}, \ldots, x_{r}$; then $A^{*}=$ $A\left[x_{1}, \ldots, x_{r}\right]$ and is Noetherian by \ref{cor:7.6}.
\end{enumerate}
\end{proof}
...
On the other hand, combining \ref{prop:10.9} with the elementary lemma \ref{lem:10.6} we obtain the really significant version:
...
\begin{remark}
In this chapter we shall apply the last part of \ref{thm:10.11} concerning topologies. However, in the next chapter the stronger result about bounded differences will be needed.
\end{remark}

As a first application of \ref{thm:10.11} we combine it with \ref{cor:10.3} to get the important exactness property of completion:
...
\begin{proof}
Using \ref{cor:10.3} or otherwise it is clear that a-adic completion commutes with finite direct sums. Hence if $F \cong A^{n}$ we have $\hat{A} \otimes_{A} F \cong \hat{F}$. Now assume $M$ is finitely generated so that we have an exact sequence
...
in which the top line is exact (by \eqref{prop:2.18}). By \ref{cor:10.3} $\delta$ is surjective. Since $\beta$ is an isomorphism this implies that $\alpha$ is surjective, proving the first part of the proposition. Assume now that $A$ is Noetherian, then $N$ is also finitely generated so that $\gamma$ is surjective and, by \ref{prop:10.12}, the bottom line is exact. A little diagram chasing now proves that $\alpha$ is injective and so an isomorphism.
\end{proof}

Propositions \ref{prop:10.12} and \ref{prop:10.13} together assert that the functor $M \mapsto$ $A \otimes_{A} M$ is exact on the category of finitely-generated $A$-modules (when $A$ is Noetherian). As shown in Chapter 2 this proves:
...
\begin{proof}
Since $A$ is Noetherian, $a$ is finitely-generated. \ref{prop:10.13} implies that the map
...
Applying \ref{cor:10.4} we now deduce

\[
A / \mathfrak{a}^{n} \cong \hat{A} / \hat{\mathbf{a}}^{n}
\]

from which iii) follows by taking quotients. By ii) and \ref{prop:10.5} we see that $\hat{A}$ is complete for its $\hat{a}$-topology. Hence for any $x \in \hat{a}$

\[
(1-x)^{-1}=1+x+x^{2}+\cdots
\]

converges in $\hat{A}$, so that $1-x$ is a unit. By \ref{prop:1.9} this implies that $\hat{\mathbf{a}}$ is contained in the Jacobson radical of $\hat{A}$.
\end{proof}
...
\begin{proof}
By \ref{prop:10.15} iii) we have $\hat{A} / \hat{\mathfrak{m}} \cong A / \mathfrak{m}$, hence $\hat{A} / \hat{\mathfrak{m}}$ is a field and so $\hat{\mathfrak{m}}$ is a maximal ideal. By \ref{prop:10.15} iv) it follows that $\hat{\mathfrak{m}}$ is the Jacobson radical of $\hat{A}$ and so is the unique maximal ideal. Thus $\hat{A}$ is a local ring.
\end{proof}
...
\begin{proof}
Since $E$ is the intersection of all neighborhoods of $0 \in M$, the topology induced on it is trivial, i.e., $E$ is the only neighborhood of $0 \in E$. By \ref{thm:10.11} the induced topology on $E$ coincides with its a-topology. Since $a E$ is a neighborhood in the a-topology it follows that $\mathfrak{a} E=E$. Since $M$ is finitely-generated and $A$ is Noetherian, $E$ is also finitely-generated and so we can apply \eqref{cor:2.5} and deduce from $\alpha E=E$ that $(1-\alpha) E=0$ for some $\alpha \in a$. The converse is obvious: if $(1-\alpha) x=0$, then
...
\begin{remark}
\begin{enumerate}[i)]
\item If $S$ is the multiplicatively closed set $1+a$, then \ref{thm:10.17} asserts that
\[
...
\end{corollary}
\begin{proof}
By \ref{prop:1.9} every element of $1+\mathfrak{a}$ is a unit.
\end{proof}

As a particularly important special case of \ref{cor:10.19} we have:
...
We can restate \ref{cor:10.20} slightly differently if we recall that an m-primary ideal of $A$ is just any ideal contained between $\mathfrak{m}$ and some power $\mathfrak{m}^{n}$ (use \eqref{prop:4.2} and \eqref{prop:7.14}). Thus \ref{cor:10.20} implies that the intersection of all m-primary ideals of $A$ is zero. If now $A$ is any Noetherian ring, $\mathfrak{p}$ a prime ideal, we can apply this version of $\ref{cor:10.20}$ to the local ring $A_{\mathfrak{p}}$. Lifting back to $A$ and using the one-to-one correspondence \eqref{prop:4.8} between $\mathfrak{p}$-primary ideals of $A$ and m-primary ideals of $A_{\mathfrak{p}}$ (where $\mathfrak{m}=\mathfrak{p} A_{\mathfrak{p}}$ ) we deduce:
...
\item $\mathfrak{a}^{n} / \mathfrak{a}^{n+1} \cong \hat{\mathfrak{a}}^{n} / \hat{\mathfrak{a}}^{n+1}$ by \ref{prop:10.15} iii).
...
From this we see, by induction on $n$, that $\operatorname{Ker} \alpha_{n}=0$ (case i)) or Coker $\alpha_{n}=0$ (case ii)). Moreover in case ii) we also have $\operatorname{Ker} \alpha_{n+1} \rightarrow \operatorname{Ker} \alpha_{n}$ surjective. Taking the inverse limit of the homomorphisms $\alpha_{n}$ and applying \ref{prop:10.2} the lemma follows.
\end{proof}

We can now form a result which is a partial converse of \ref{prop:10.22} iii) and is the main step in showing that $\hat{A}$ is Noetherian.
...
of filtered groups, and $G(\phi): G(F) \rightarrow G(M)$ is a homomorphism of $G(A)$-modules. By construction it is surjective. Hence by $\ref{lem:10.23}$ ii) $\hat{\phi}$ is surjective. Consider now the diagram
...
\begin{proof}
By \ref{prop:10.22} we know that
\[
G_{\mathfrak{a}}(A)=G_{\mathfrak{a}}(\hat{A})
\]
is Noetherian. Now apply \ref{cor:10.25} to the complete ring $\hat{A}$, taking $M=\hat{A}$ (filtered by $\hat{a}^{n}$, and so Hausdorff).
\end{proof}
...
\]
and apply \ref{prop:10.13}. Then use the isomorphism
\[
...
\]
now use \ref{cor:10.19} and Exercise 6.]
...
\item Show that the converse of \ref{thm:10.26} is false, even if we assume that $A$ is local and that $\hat{A}$ is a finitely-generated $A$-module.
