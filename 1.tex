...
If $f: A \to B$ is any ring homomorphism, the \index{Kernel}kernel of
$f\left(=f^{-1}(0)\right)$ is an ideal a of $A$, and the \index{Image}image of $f$ ($=f(A)$)
...
zero-divisors $\neq 0$ (and in which $1 \neq 0$) is called an \index{Domain!integral}integral domain.
For example, $\mathbf{Z}$ and $k\left[x_{1}, \ldots, x_{n}\right]$ ( $k$ a
field, $x_{i}$ indeterminates) are integral domains.

An element $x \in A$ is \index{Nilpotent}nilpotent if $x^{n}=0$ for some $n>0$. A nilpotent
element is a zero-divisor (unless $A=0$), but not conversely (in general).

A \index{Unit}unit in $A$ is an element $x$ which ``divides 1'', i.e., an element $x$ such
...
group. The multiples $a x$ of an element $x \in A$ form a \index{Ideal!principal}principal ideal,
denoted by $(x)$ or $A x$. $x$ is a unit $\iff$ $(x)=A=(1)$. The zero ideal
$(0)$ is usually denoted by $0$.

A \index{Field}field is a ring $A$ in which $1 \neq 0$ and every non-zero element is a unit.
...
An ideal $\mathfrak{p}$ in $A$ is \index{Ideal!prime}prime if $\mathfrak{p} \neq(1)$ and if
$x y \in \mathfrak{p} \implies x \in \mathfrak{p}$ or $y \in \mathfrak{p}$.

An ideal $\mathfrak{m}$ in $A$ is \index{Ideal!maximal}maximal if $\mathfrak{m} \neq(1)$ and if there
...
\end{theorem}
\begin{proof}
  This is a standard application of \index{Zorn's lemma}Zorn's lemma. \footnote{Let $S$ be a
...
\begin{remark}
  \begin{enumerate}
    ...
    \item There exist rings with exactly one maximal ideal, for example fields.
          \index{Local ring}A ring $A$ with exactly one maximal ideal $m$ is called a \index{Ring!local}local ring.
          The field $k=A / \mathfrak{m}$ is called the \index{Field!residue}residue field of $A$.
  \end{enumerate}
\end{remark}
...
\begin{proof}
  ...
  \end{enumerate}
\end{proof}
A ring with only a finite number of maximal ideals is called \index{Ring!semi-local}semi-local.
\begin{example}
  \begin{enumerate}[1.]
...
If $\mathfrak{a}, \mathfrak{b}$ are ideals in a ring $A$, their \index{Ideal!sum}sum
...
\end{enumerate}
  ...
  \item A ring $A$ is \index{Boolean ring}Boolean if $x^{2}=x$ for all $x \in A$. In a Boolean ring
  $A$, show that
  \begin{enumerate}[i)]
...
polynomial function on $X$. Hence $\phi$ induces a $k$-\index{Algebra!homomorphism}algebra homomorphism
