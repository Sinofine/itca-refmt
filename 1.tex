...
\begin{remark}
  \begin{enumerate}
    ...
    \item There exist rings with exactly one maximal ideal, for example fields.
          \index{Local ring}A ring $A$ with exactly one maximal ideal $\mathfrak{m}$ is called a \index{Ring!local}{\itshape local ring}.
...
\begin{proposition}\label{prop:1.6}
  \begin{enumerate}[i)]
    ...
    \item Let $A$ be a ring and $\mathfrak{m}$ a maximal ideal of $A$, such that every
