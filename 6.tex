...
If $\Sigma$ is the set of submodules of a module $M$, ordered by the relation $\subseteq$, then i) is called the ascending chain condition (a.c.c. for short) and ii) the maximal condition. A module $M$ satisfying either of these equivalent conditions is said to be Noetherian\index{Module!Noetherian}\index{Noetherian!module} (after Emmy Noether). If $\Sigma$ is ordered by $\supseteq$, then i) is the descending chain condition (d.c.c. for short) and ii) the minimal condition. A module $M$ satisfying these is said to be Artinian\index{Module!Artinian}\index{Artinian!module} (after Emil Artin).
...
A ring $A$ is said to be Noetherian\index{Noetherian!ring} (resp. Artinian\index{Artinian!ring}) if it is so as an $A$-module, i.e., if it satisfies a.c.c. (resp. d.c.c.) on ideals.
...
A chain of submodules\index{Chain of submodules} of a module $M$ is a sequence $\left(M_{i}\right)(0 \leqslant i \leqslant n)$ of submodules of $M$ such that

\[
M=M_{0} \supset M_{1} \supset \ldots \supset M_{n}=0 \text { (strict inclusions). }
\]

The length\index{Length} of the chain is $n$ (the number of "links"). A composition series\index{Composition series} of $M$ is a maximal chain, that is one in which no extra submodules can be inserted: this is equivalent to saying that each quotient $M_{t-1} / M_{t}(1 \leqslant i \leqslant n)$ is simple (that is, has no submodules except 0 and itself).
